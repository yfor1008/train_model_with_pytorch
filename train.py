...
# train task
parser.add_argument('--proj-name', default='sdql', type=str,
                    help='name of train task project')
...
def main():
    ...
    # 记录实验数据
    task = Task.init(project_name='%s' % args.proj_name, task_name='train_%s_%d' % (args.model, int(time.time())))
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            eval_metrics = validate(model, loader_eval, validate_loss_fn, args, epoch, amp_autocast=amp_autocast)

            if model_ema is not None and not args.model_ema_force_cpu:
                if args.distributed and args.dist_bn in ('broadcast', 'reduce'):
                    distribute_bn(model_ema, args.world_size, args.dist_bn == 'reduce')
                ema_eval_metrics = validate(
                    model_ema.module, loader_eval, validate_loss_fn, args, epoch, amp_autocast=amp_autocast, log_suffix=' (EMA)')
